...
//! rosecallpathhpct/xml2profir/xml2profir.hh included.
...
{
//! \addtogroup ROSECALLPATHHPCT_XML2PROFIR
//@{
...
//! Returns a lookup table of short-to-long load modul names.
std::map<std::string, std::string> getLoadModuleTable(const XMLNode_t *doc);
//! Returns a lookup table of short-to-long file names.
std::map<std::string, std::string> getFileTable(const XMLNode_t *doc);
//! Returns a lookup table of short-to-long procedure names.
std::map<std::string, std::string> getProcedureTable(const XMLNode_t *doc);
...
typedef std::vector<RoseCallPathHPCT::IRTree_t *> ProgramTreeList_t;
...
bool translateIdsToNames(IRTree_t * root, const std::map<std::string, std::string> & metrics, 
...
void collectMetricNames(const IRTree_t *root,
                        std::map<std::string, unsigned int> &metrics);
