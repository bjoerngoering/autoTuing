...
#if !defined(INC_SAGE_ROSECALLPATHHPCT_HH)
//! rosehpct/sage/sage.hh included.
#define INC_SAGE_ROSECALLPATHHPCT_HH

#include <iostream>
#include <string>
#include <vector>

#include <rosecallpathhpct/sage/collectors.hh>
#include "rose.h"
...
namespace RoseCallPathHPCT
{
  ...
  //  std::string attribute_class_name() { return "MetricAttr"; }
    ...
    virtual void unpacked_data(int size, char* data)
    {
      ...
      //1. check tail **
      char * head = data;
      char * tail = head + (std::string(head)).size() - 1;
      if (*tail=='*')
        is_derived_ = true;
