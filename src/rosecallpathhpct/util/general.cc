...
string
GenUtil::toFileLoc (const string& filename, unsigned int line_start, unsigned int line_end)
{
  ostringstream o;
  o << filename;
  if (line_start > 0)
...
string
GenUtil::tabs (unsigned int n, unsigned int tab_width)
{
  string s = "";
  for (unsigned int i = 0; i < n * tab_width; i++)
    s += " ";
  return s;
}
