 ...
 *  \brief Converts an XML HPCToolkit profile file into an RoseCallPathHPCTIR
 *  tree.
...
#include "rosecallpathhpct/util/types.hh"
 ...
 *  to the RoseCallPathHPCT IR representation.
 *
...
void XMLToProfIRConverter::visit(TreeParamPtr_t tree)
{
  ...
  if (prog_root_ == NULL)
  {
    ...
    IRTree_t::Val_t v =
        ProfIRFactory_t::instance().createObject("SecCallPathProfile");
    if (v != NULL)
    {
  ...
  //std::cout << "new value is: " << e.name << " tree value name:" << tree->value.name << std::endl;
  assert(new_val != NULL);

  size_t num_kids = parent->getNumChildren();
  num_kids++;
  IRTree_t *child = parent->setChildValue(num_kids, new_val);

  saved_parents_.push(child);
}

void XMLToProfIRConverter::ascending(void)
{
  if (saved_parents_.size() > 0)
    saved_parents_.pop();
}
...
IRTree_t *
RoseCallPathHPCT::convertXMLToProfIR(const XMLNode_t *root)
{
