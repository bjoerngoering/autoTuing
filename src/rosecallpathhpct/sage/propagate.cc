...
string
RoseCallPathHPCT::getFilename (const SgLocatedNode* node)
{
  return (node == NULL) ? "" : ::getFilename (node->get_startOfConstruct ());
}

int
RoseCallPathHPCT::getLineStart (const SgLocatedNode* node)
{
  return (node == NULL) ? 0 : ::getLineNumber (node->get_startOfConstruct ());
}

int
RoseCallPathHPCT::getLineEnd (const SgLocatedNode* node)
{
  return (node == NULL) ? 0 : ::getLineNumber (node->get_endOfConstruct ());
}

string
RoseCallPathHPCT::toFileLoc (const SgLocatedNode* node)
{
  if (node == NULL) return "";
			     ...
			     const SynthesizedAttributesList& attr_list)
{
  ...
  if (total.getValue () > 0)
    if (total.getName () == metric_name_)
      {
        if (RoseCallPathHPCT::enable_debug)
        {
          std::ostringstream o;
...
void
RoseCallPathHPCT::propagateMetrics (SgProject* node,
