...
#include "rosecallpathhpct/sage/sage.hh"
...
FileCollector::matches (SgGlobal* node) const
{
  ...
  string sg_fullpath = RoseCallPathHPCT::getFilename (node);
...
StmtCollectorByLoc::matches (const SgForInitStatement* node) const
{
   ...
   */
  string filename = RoseCallPathHPCT::getFilename (node);
  int line_start = RoseCallPathHPCT::getLineStart (loop);
  int line_end = RoseCallPathHPCT::getLineEnd (loop->get_loop_body ()) - 1;
...
void Vis_PrintMetricInfo::printAttributes(SgLocatedNode* node)
{
      ...
      if (astAttributeContainer != NULL)
      {
        ...
        {
           RoseCallPathHPCT::MetricAttr* attribute = dynamic_cast<RoseCallPathHPCT::MetricAttr *>(astAttributeContainer->operator[](name));
