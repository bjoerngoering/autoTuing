...
#include "rosecallpathhpct/profir2sage/profir2sage.hh"
...
ROSE_DLL_API std::set<const RoseCallPathHPCT::IRNode *> profFileNodes_;
ROSE_DLL_API std::set<const RoseCallPathHPCT::IRNode *> profStmtNodes_;
...
ROSE_DLL_API std::map<const RoseCallPathHPCT::IRNode *, std::set<SgLocatedNode *> > profSageMap_;
}
...
static
bool doLinesOverlap(const SgLocatedNode* node, const RoseCallPathHPCT::Located * plnode)
 ...
 */
class MetricFinder: public Trees::Traversal<RoseCallPathHPCT::IRTree_t, true>
{
public:
    typedef std::set<const RoseCallPathHPCT::IRNode *> MatchSet_t;
    ...
    MetricFinder(const SgLocatedNode* target, std::map<const RoseCallPathHPCT::IRNode *, std::set<SgLocatedNode *> > * historyRecord);
...
#if 1
 // not needed as already used
    /*! \brief Handles LoadModule nodes. */
    virtual void visit(RoseCallPathHPCT::LoadModule* node);
    /*! \brief Handles File nodes. */
    virtual void visit(RoseCallPathHPCT::File* node);
    /*! \brief Handles Procedure nodes. */
    virtual void visit(RoseCallPathHPCT::Procedure* node);
    /*! \brief Handles Proc nodes. */
    virtual void visit(RoseCallPathHPCT::Proc* node);
    /*! \brief Handles ProcFrame nodes. */
    virtual void visit(RoseCallPathHPCT::ProcFrame* node);
    /*! \brief Handles Loop nodes. */
    virtual void visit(RoseCallPathHPCT::Loop* node);
    /*! \brief Handles Statement nodes. */
    virtual void visit(RoseCallPathHPCT::Statement* node);

    bool doLinesOverlap(RoseCallPathHPCT::Located* l) const;
    ...
    bool isShadowNode(RoseCallPathHPCT::IRNode* profNode) const;

#else

    /*! \brief Handles File nodes. */
    virtual void visit (const RoseCallPathHPCT::File* node);
    /*! \brief Handles Procedure nodes. */
    virtual void visit (const RoseCallPathHPCT::Procedure* node);
    /*! \brief Handles Loop nodes. */
    virtual void visit (const RoseCallPathHPCT::Loop* node);
    /*! \brief Handles Statement nodes. */
    virtual void visit (const RoseCallPathHPCT::Statement* node);

    bool doLinesOverlap (const RoseCallPathHPCT::Located* l) const;
    //bool doLinesOverlap (size_t a, size_t b) const;

    /*! \brief Check if the target node is a shadow node of
     * the previously matched nodes to a profile node */
    bool isShadowNode(const RoseCallPathHPCT::IRNode* profNode) const;
...
private:
    ...
    MatchSet_t procedure_table_;
    MatchSet_t file_table_;
    MatchSet_t loadmodule_table_;
...
protected:
    ...
    std::map<const RoseCallPathHPCT::IRNode *, std::set<SgLocatedNode *> > * historyRecord_; //! A map between IRNode to matched SgNodes, used to avoid redundant attaching for code expanded from macro, Liao, 2/3/2009
...
{
public:
    MetricAttachTraversal(const RoseCallPathHPCT::IRTree_t* hpc_root);
    void setVerbose(bool enable = false);
    ...
    typedef std::map<const RoseCallPathHPCT::IRNode *, SgLocNodeSet_t> AttachedNodes_t;
    AttachedNodes_t& getAttachedNodes(void);
#if 0  
    std::set<const RoseCallPathHPCT::IRNode *>& getProfFileNodes (void);
    std::set<const RoseCallPathHPCT::IRNode *>& getProfStmtNodes (void);
#endif  
...
private:
    const RoseCallPathHPCT::IRTree_t* hpc_root_; //!< Root of profiling data tree
    AttachedNodes_t attached_; //!< Lists of attached nodes
#if 0  
    std::set<const RoseCallPathHPCT::IRNode *> files_; //!< set of all profir file nodes
    std::set<const RoseCallPathHPCT::IRNode *> stmts_;//!< set of all profir statement nodes
#endif  
...
MetricFinder::MetricFinder(const SgLocatedNode* target, std::map<const RoseCallPathHPCT::IRNode *, SgLocNodeSet_t> * historyRecord) :
...
#if 1
bool MetricFinder::doLinesOverlap(RoseCallPathHPCT::Located * l) const
#else
        bool
        MetricFinder::doLinesOverlap (const RoseCallPathHPCT::Located * l) const
#endif
{
...
// SgNode to the current RoseCallPathHPCT::IRNode (profNode) node. 
//
...
// Liao, 2/3/2009
#if 1
bool MetricFinder::isShadowNode(RoseCallPathHPCT::IRNode* profNode) const
#else
        bool
        MetricFinder::isShadowNode(const RoseCallPathHPCT::IRNode* profNode) const
#endif
{
...
{
 if (isTargetSgProcedure() &&  dynamic_cast<Proc *>(tree->value))
        visit(dynamic_cast<Proc *>(tree->value));
    else if (isTargetSgProcedure() &&  dynamic_cast<ProcFrame *>(tree->value))
        visit(dynamic_cast<ProcFrame *>(tree->value));
...
#endif
{
        ...
        RoseCallPathHPCT::profFileNodes_.insert(f);
#endif    
...
// #if 1
// void MetricFinder::visit(LoadModule* f)
// #else
// void
// MetricFinder::visit (const LoadModule* f)
// #endif
// {
// #if 0  // moved to pathFinder traversal   
//         //take advantage of this traversal to accumulate file nodes
//         RoseCallPathHPCT::profFileNodes_.insert(f);
// #endif   // TODO do loadmodule name matches
//         if (!doFilenamesMatch(target_, f->getName()))
//             prune_branch_ = false; //  DXN
//         else // match LoadModule name
//         {
//             // match SgNode type: looking for a LoadModule (global) scope
//                 //matches_.insert(f);
//                 loadmodule_table_.insert(f);
...
// #if 1
// void MetricFinder::visit(Procedure* f)
// #else
// void
// MetricFinder::visit (const File* f)
// #endif
...
//         //take advantage of this traversal to accumulate file nodes
//         RoseCallPathHPCT::profFileNodes_.insert(f);
// #endif     //TODO add do procedure names match
//         if (!doFilenamesMatch(target_, f->getName()))
//             prune_branch_ = false; //  DXN
//         else // match file name
//         {
...
#if 1
void MetricFinder::visit(Proc* p)
#else
void
...
#if 1
void MetricFinder::visit(ProcFrame* p)
#else
void
MetricFinder::visit (const Procedure* p)
#endif
...
/*! TODO maybe use as metrics are only at bottom level
...
{
#if defined(DO_ATTACH_LOOP_BODIES) //TODO define it
    if (loop && attr_sage)
    {
...
#if 0
std::set<const RoseCallPathHPCT::IRNode *>&
...
void MetricAttachTraversal::annotateSourceCode(void)
{
    ...
    {
        const RoseCallPathHPCT::IRNode * profileNode = (*iter).first;
...
void RoseCallPathHPCT::attachMetrics(const IRTree_t* hpc_root, SgProject* sage_root, bool verbose)
{
    ...
    std::set<const RoseCallPathHPCT::IRNode *>&files = attacher.getProfFileNodes();
    std::set<const RoseCallPathHPCT::IRNode *>::iterator iter= files.begin();
    for (;iter!=files.end(); iter++)
    profFileNodes_.insert(*iter);
    // Accumulate non-scope stmt metrics, used to find hot stmts later on
    std::set<const RoseCallPathHPCT::IRNode *>&stmts= attacher.getProfStmtNodes();
    std::set<const RoseCallPathHPCT::IRNode *>::iterator iter2= stmts.begin();
